...
- **Deploy / infra:** partial — live on Vercel at https://www.chrobok.dev via the Git integration, but `.github/workflows/` does not exist and there is no `vercel.json`. As of 2026-07-23 there is no `.github/` directory at all — it held only the retired Copilot instruction set — so F-03 creates it from scratch. Vercel's Production Branch is `main` — verified 2026-07-23 against the GitHub Deployments API, which shows `origin/main` deploying to Production and `origin/development` to Preview. Merging to `main` therefore publishes straight to the live site with no server-side gate in front of it; merging to `development` does not.
...
- **Agent instructions:** present — `CLAUDE.md` at the repo root (project facts, the `main`-is-production and yarn-only rules, structure, naming) plus `.claude/rules/` (`release-process.md` unscoped so it loads every session; `typescript`, `react`, `astro`, `tailwind`, `design-system`, `ui-shadcn` scoped via `paths:` frontmatter). Migrated 2026-07-23 from `.github/instructions/`; before that the rules existed but never loaded, because they used Copilot's `applyTo:` convention and there was no `CLAUDE.md`.
